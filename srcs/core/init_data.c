...
{
	game->dir_infos[0] = (t_dir_info){DIR_NORTH_CHAR, DIR_NORTH_X, DIR_NORTH_Y,
		DIR_NORTH_PLANE_X, DIR_NORTH_PLANE_Y};
	game->dir_infos[1] = (t_dir_info){DIR_SOUTH_CHAR, DIR_SOUTH_X, DIR_SOUTH_Y,
		DIR_SOUTH_PLANE_X, DIR_SOUTH_PLANE_Y};
	game->dir_infos[2] = (t_dir_info){DIR_EAST_CHAR, DIR_EAST_X, DIR_EAST_Y,
		DIR_EAST_PLANE_X, DIR_EAST_PLANE_Y};
	game->dir_infos[3] = (t_dir_info){DIR_WEST_CHAR, DIR_WEST_X, DIR_WEST_Y,
...
void	init_player(t_game *game)
{
	...
	i = 0;
	game->player_x = game->cfg->player_x + RAY_HALF_TILE_OFFSET;
	game->player_y = game->cfg->player_y + RAY_HALF_TILE_OFFSET;
	while (i < DIR_COUNT)
	{
	...
	while (game->cfg->map[y])
	{
		...
		while (game->cfg->map[y][x])
		{
			if (game->cfg->map[y][x] == TILE_DOOR)
				game->doors[idx++] = (t_door){x, y, DOOR_OPEN_RATIO_START, 0};
			x++;
		}
		y++;
	}
