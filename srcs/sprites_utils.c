...
void	draw_sprite_stripe(t_game *g, t_sprite *s, float *zb)
{
	...
	while (++x <= s->end_x)
	{
		...
		while (++y <= s->end_y)
		{
			if (y < 0 || y >= HEIGHT)
				continue ;
			tx = (x - s->start_x) * tex->width / s->width;
			ty = (y - s->start_y) * tex->height / s->height;
			c = get_texture_color(g, TEX_SPRITE, tx, ty);
			a = (c >> 24) & 0xFF;
			if (a == 0)
				continue ;
			r  = (c      ) & 0xFF;
			...
			b  = (c >> 16) & 0xFF;
			if (a == 255)
				dst[y * WIDTH + x] = 0xFF000000u | (r << 16) | (gc << 8) | b;
			else
			{
				c = dst[y * WIDTH + x];
				r  = (uint8_t)((r * a + ((c >> 16) & 0xFF) * (255 - a)) / 255);
				gc = (uint8_t)((gc * a + ((c >>  8) & 0xFF) * (255 - a)) / 255);
				b  = (uint8_t)((b * a + (c & 0xFF) * (255 - a)) / 255);
				dst[y * WIDTH + x] = 0xFF000000u | (r << 16) | (gc << 8) | b;
			}
		}
	}
